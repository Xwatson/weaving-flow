...
export class ScriptExecuteNode extends BaseNode {
  ...
  constructor(name: string, config: Record<string, any> = {}) {
    super(name, 'scriptExecute', config);
  }
  ...
  getInputDefinitions(): NodeInput[] {
    return [
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: '验证码检测脚本，需要返回布尔值：true表示存在验证码，false表示无验证码或验证码已处理',
      },
      {
        name: 'captchaTimeout',
        type: 'number',
  ...
  private async waitForCaptcha(browser: any, checkScript: string): Promise<boolean> {
      ...
      while (shouldContinue) {
        ...
        if (enableCaptchaCheck && captchaCheckScript) {
          ...
          while (await this.waitForCaptcha(browser, captchaCheckScript)) {
            ...
            // 等待用户处理验证码
            await new Promise(resolve => setTimeout(resolve, captchaCheckInterval));
          }
        }
